  ...
  persist(
    (set, get) => ({
      ...
      setCurrency: (currency) => {
        set({ currency, isAutoDetected: true });
      },

      // Initialize / auto-detect user's country if not manually chosen
      initCurrency: async () => {
        const state = get();
        // If user already explicitly set currency or already detected, keep it
        if (state.isAutoDetected && state.currency) {
          return;
        }
        ...
        // Default fallback
        set({ currency: "PKR", country: "PK", isAutoDetected: true });
      },
    ...
    {
      name: "jannah-currency-preference",
    }
  )
);
